...
namespace :deploy do
  ...
  before 'deploy:symlink:linked_files', :detect_scd_config do
    on primary fetch(:magento_deploy_setup_role) do
      unless test %Q[#{SSHKit.config.command_map[:php]} -r '
            $cfg = include "#{release_path}/src/app/etc/config.php";
        ...
        remove :linked_files, 'src/app/etc/env.php'
      end
    end
  end
  ...
  task 'symlink:link_env_php' do
    on release_roles :all do
      ...
      execute :ln, "-fsn #{shared_path}/src/app/etc/env.php #{release_path}/src/app/etc/env.php"
    end
  end
